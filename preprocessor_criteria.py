...
def start(company_code):
    ...
    with open("./criteria_news/" + company_code + '.csv', 'r', -1, 'utf-8') as news:
        next(news)
    ...
    f = open("./criteria_words/" + company_code + '.csv', "w+")
    f.close()
    ...
    df.to_csv("./criteria_words/" + company_code + '.csv', index=False)
    pd.read_csv("./criteria_words/" + company_code + '.csv')

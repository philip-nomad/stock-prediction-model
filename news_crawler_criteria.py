...
PATH = "./"
os.chdir(PATH)
if not os.path.exists('criteria_news'):
    os.makedirs('criteria_news')

if not os.path.exists('criteria_score'):
    os.makedirs('criteria_score')

if not os.path.exists('criteria_words'):
...
def start(company_code,days,wherep):
    ...
    df_result.to_csv("./criteria_news/" + company_code + '.csv', mode='w', encoding='utf-8-sig')

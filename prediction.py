...
def start(company_code, learning_date):
    print(f"company_code: {company_code} {learning_date}의 다음 날 종가 예측 시작")

    # LSTM 값 불러오기
    with open(f"./{lstm_calculator.DIR}/{company_code}/{company_code}_{learning_date}.csv", 'r', -1, 'utf-8') as lines:
        next(lines)

        for line in csv.reader(lines):
            lstm_prediction = round(float(line[0]))
            closing_price = round(float(line[1]))

    lstm_value = (lstm_prediction - closing_price) / closing_price  # (다음날 예측 종가 - 오늘 종가) / 오늘 종가
    print(f"LSTM 예측 변동률: {lstm_value * 100}%")

    # 감성분석 값 불러오기
    emotional_analysis_value = float(
        news_contents_sentimental_analysis.calculate_two_weeks(company_code, learning_date))
    print(f"감성 분석을 이용한 예측 변동률: {emotional_analysis_value * 100}%")
    ...
    with open('./per_data/csv/' + company_code + '.csv', 'r', -1, 'utf-8') as lines:
        ...
        for line in csv.reader(lines):
            company_per = float(line[2])
            same_category_per = float(line[3])
    ...
    if company_per > 0 and same_category_per > 0:
        ...
        per_value = 0
    print(f"PER 을 이용한 점수: {per_value}")

    # 가중치 a, b, c 곱해서 예측점수 구하기
    predicted_value = (lstm_value * WEIGHT_FOR_LSTM_VALUE +
                       float(emotional_analysis_value) * WEIGHT_FOR_EMOTIONAL_ANALYSIS_VALUE +
                       per_value * WEIGHT_FOR_PER_VALUE)

    print(f"최종 예측 변동률(감성분석 + LSTM + PER): {predicted_value * 100}%\n")
    return predicted_value


if __name__ == '__main__':
    start('035720', datetime.date(2021, 5, 9))
